...
public:
  ...
  void popThirdFromTop();
  void pushTwoUnderTop(const T& input);
  T topThirdFromTop();
...
StackForCS2420<T>::StackForCS2420(const unsigned int capacity) {
  cout << "messages with \"insufficient\" are just warnings, not errors" << endl;
...
template <typename T>
void StackForCS2420<T>::push(const T& input) {
...
void StackForCS2420<T>::pop() {
  if (this->index > 0) {
    ...
    cerr << "insufficient index at pop()" << endl;
  }
}
...
T StackForCS2420<T>::top() {
  if (this->index > 0) {
    ...
    cerr << "insufficient index at top()" << endl;
    throw 1;
    return 0;
  }
}
...
void StackForCS2420<T>::popThirdFromTop() {
  if (this->index < 3) {
    cerr << "insufficient index at popThirdFromTop()" << endl;
  } else {
    int t1 = top();
    pop();
    int t2 = top();
...
template <typename T>
void StackForCS2420<T>::pushTwoUnderTop(const T& input) {
  if (this->index < 3) {
    cerr << "insufficient index at pushTwoUnderTop()" << endl;
  } else {
    int t1 = top();
    pop();
    int t2 = top();
...
T StackForCS2420<T>::topThirdFromTop() {
  if (this->index < 3) {
    cerr << "insufficient index at topThirdFromTop()" << endl;
    return 2;
  } else {
    pop();
    pop();
