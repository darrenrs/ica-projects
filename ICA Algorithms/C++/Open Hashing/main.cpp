...
public:
  ...
  T retrieve(string key) const;
  T& operator[](string key) const;
  void remove(string key) const;
...
template <typename T>
void HashTable<T>::create(const string key, const T value) const {
  int index = hash<string>{}(key) % this->buckets;

  pair<string, T> pairObject(key, value);

  this->arr[index].push_front(pairObject);
}
...
void HashTable<T>::create(const string key, T&& value)  {
  ...
  this->arr[index].emplace_front(key, std::move(value));
}

template <typename T>
bool HashTable<T>::exists(string key) const {
  int index = hash<string>{}(key) % this->buckets;

  bool status = false;

  for (auto i = this->arr[index].begin(); i != this->arr[index].end(); ++i) {
    status = true;
  }
...
template <typename T>
T HashTable<T>::retrieve(string key) const {
  int index = hash<string>{}(key) % this->buckets;

  return this->arr[index].front().second;
}

template <typename T>
T& HashTable<T>::operator[](string key) const {
  int index = hash<string>{}(key) % this->buckets;
...
template <typename T>
void HashTable<T>::remove(string key) const {
  int index = hash<string>{}(key) % this->buckets;

  this->arr[index].remove( this->arr[index].front() );
}
