...
BaseDoublyLinkedList<T>::~BaseDoublyLinkedList() {
  // SORRY, I had to add in the next three lines lol
  if (first == nullptr) {
    return;
  }
  while (first != last) {
...
T DoublyLinkedList<T>::get(const unsigned int index) const {
  int currentIndex = 0;
  bool complete = false;
  shared_ptr<Node<T>> currentNode{ BaseDoublyLinkedList<T>::first };
  ...
  while (!complete) {
    ...
    if (currentNode->next == nullptr) {
      //cout << "invalid const unsigned int index at T get(); exiting loop at line 222" << endl;
      throw 1;
    } else {
      currentNode = currentNode->next;
      currentIndex++;
...
T& DoublyLinkedList<T>::operator[](const unsigned int index) const {
  ...
  while (!complete) {
    ...
    if (currentNode->next == nullptr) {
      //cout << "invalid const unsigned int index at T[]; exiting loop at line 242" << endl;
      throw 1;
    } else {
      currentNode = currentNode->next;
      currentIndex++;
...
template <typename T>
void DoublyLinkedList<T>::insert(const unsigned int index, const T& value) {int currentIndex = 0;
  bool complete = false;
  shared_ptr<Node<T>> currentNode{ BaseDoublyLinkedList<T>::first };
  ...
  while (!complete) {
    if (currentIndex + index == 0) {
      ...
      if (currentNode == nullptr) {
        // no next entry
        shared_ptr<Node<T>> lle = make_shared<Node<T>>();
        lle->data = value;
        ...
        BaseDoublyLinkedList<T>::first = lle;
        BaseDoublyLinkedList<T>::last = lle;
        ...
        lle->prev = nullptr;
        lle->next = currentNode;
        currentNode->prev = lle;

        BaseDoublyLinkedList<T>::first = lle;
        
        break;
      }
    } else if (currentIndex == index - 1 && currentNode->next == nullptr) {
      // appending to end
      shared_ptr<Node<T>> lle = make_shared<Node<T>>();
      lle->data = value;

      lle->prev = currentNode;
      lle->next = nullptr;
      currentNode->next = lle;

      BaseDoublyLinkedList<T>::last = lle;

      break;
    } else if (currentIndex == index - 1) {
      // appending within linked list
      shared_ptr<Node<T>> lle = make_shared<Node<T>>();
      lle->data = value;

      lle->prev = currentNode;
      lle->next = currentNode->next;
      (currentNode->next)->prev = lle;
      currentNode->next = lle;

      break;
    }
    if (currentNode->next == nullptr) {
      //cout << "invalid const unsigned int index at T insert(); exiting loop at line 309" << endl;
      return;
    } else {
      currentNode = currentNode->next;
      currentIndex++;
...
void DoublyLinkedList<T>::remove(const unsigned int index) {
  ...
  while (!complete) {
    if (currentNode != nullptr) {
      if ((currentNode->next) == nullptr) {
        if (index == 0) {
          BaseDoublyLinkedList<T>::first = nullptr;
          currentNode = nullptr;
        }
        break;
      } else if (currentIndex + index == 0) {
        BaseDoublyLinkedList<T>::first = currentNode->next;
        (currentNode->next)->prev = nullptr;

        break;
      } else if (currentIndex == index - 1 && (currentNode->next)->next == nullptr) {
        BaseDoublyLinkedList<T>::last = currentNode;
      ...
      if (currentNode->next == nullptr) {
        //cout << "invalid const unsigned int index at t remove(); exiting loop at line 349" << endl;
        return;
      } else {
        currentNode = currentNode->next;
        currentIndex++;
  ...
  while (!complete) {
    if (currentNode != nullptr) {
      if (currentNode->data == value) {
        //cout << "Match found " << currentIndex << endl;
        ...
        if (currentNode->prev == nullptr && currentNode->next == nullptr) {
          ...
          BaseDoublyLinkedList<T>::last = currentNode->prev;
        } else {
      ...
      if (currentNode->next == nullptr) {
        //cout << "invalid const unsigned int index at removeAllInstances(); exiting loop at line 388" << endl;
        return;
      } else {
        currentNode = currentNode->next;
        currentIndex++;
