...
bool Iterator<T>::operator!=(const Iterator<T>& rhs) const {
  ...
  return (rhs.hashTable != this->hashTable ||
...
Iterator<T> Iterator<T>::operator++() {
  ...
  for (int i = index + 1; i < capacity; i++) {
    if (this->hashTable->status_arr[i] == 1) {
      //cout << "New Index: " << i << endl;
      this->index = i;
      break;
    }
    if (i + 1 == capacity) {
      //cout << "New Index: " << capacity << endl;
      this->index = capacity;
      break;
    }
  }
...
//************************************************************
// Put HashTable constructors and methods below here
//************************************************************
...
void HashTable<T>::create(const string& key, const T& item) {
  double next = this->count + 1.0;
  if ((next / (double)capacity) > (double)0.75) {
    // rehash
    HashTable<T>* hashTable = new HashTable(capacity * 2);

    for (int i = 0; i < capacity; i++) { 
      if (this->status_arr[i] == 1) {
        hashTable->create(this->keyValue_arr[i].first,
                          this->keyValue_arr[i].second);
      }
    }
  ...
  for (int h = 0; h < capacity; h++) {
    ...
    if (h == 0) {
      index = std::hash<string>{}(key) % this->capacity;
    }

    for (int i = index; i < capacity; i++) {
      if (this->status_arr[i] <= 0) {
        this->keyValue_arr[i].first = key;
        this->keyValue_arr[i].second = item;
...
void HashTable<T>::create(const string& key, T&& item) {
  ...
  for (int h = 0; h < capacity; h++) {
    ...
    for (int i = index; i < capacity; i++) {
      if (this->status_arr[i] <= 0) {
        this->keyValue_arr[i].first = std::move(key);
        this->keyValue_arr[i].second = std::move(item);
...
T& HashTable<T>::operator[](const string& key) {
  for (int h = 0; h < capacity; h++) {
    ...
    for (int i = index; i < capacity; i++) {
      if (this->status_arr[i] > 0 && (std::hash<string>{}(key) % this->capacity == index)) {
        this->status_arr[i] = 1;
        return this->keyValue_arr[i].second;
      }
    }
  }
  // SHOULD NOT REACH THIS LINE
  return this->keyValue_arr[0].second;
}
...
T HashTable<T>::retrieve(const string& key) {
  for (int h = 0; h < capacity; h++) {
    ...
    for (int i = index; i < capacity; i++) {
      if (this->status_arr[i] > 0 && (key == this->keyValue_arr[i].first)) {
        this->status_arr[i] = 1;
        return this->keyValue_arr[i].second;
      }
    }
...
void HashTable<T>::remove(const string& key) {
  for (int h = 0; h <= 1; h++) {
    ...
    for (int i = index; i < capacity; i++) {
      if (this->status_arr[i] > 0 && (std::hash<string>{}(key) % this->capacity == i)) {
        this->status_arr[i] = -1;
        this->count--;
        return;
      }
...
Iterator<T> HashTable<T>::begin() {
  ...
  for (int i = 0; i < this->capacity; i++) {
    if (this->status_arr[i] == 1) {
      ...
      return *iterator;
      break;
    } else if (i + 1 == this->capacity) {
      iterator->index = this->capacity;
      return *iterator;
      break;
    }
  }
...
//************************************************************
// Put HashTable constructors and methods above here
//************************************************************
//Copyright 2020, Bradley Peterson, Weber State University, all rights reserved. (10/20)
...
void testHashOfObjects() {
  ...
  //Test #1, add in a studentObject.  Try to retrieve it.
  Product tempProduct;
...
void testIteration() {
  ...
  if (iter != anotherIter) {
   cout << "****** Failed testIteration #1 ****** " << endl << "   The two iterators held the same data." << endl;
  }
  else {
  ...
  if (iter != anotherIter) {
   ...
   cout << "****** Failed testIteration #2 ****** " << endl << "     The two iterators held different data." << endl;
  }
  ...
  if (iter != endIter) {
   cout << "****** Failed testIteration #3 ****** " << endl << "   The two iterators are the same, and != should have returned false." << endl;
  }
  else {
